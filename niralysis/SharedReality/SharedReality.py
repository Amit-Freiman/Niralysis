...
TABLE_ROWS = [A_PRE_CHOICE_1, A_PRE_CHOICE_2, B_PRE_CHOICE_1, B_PRE_CHOICE_2, POST_CHOICE_1, POST_CHOICE_2, CONTROL_1, CONTROL_2]
class SharedReality:
    ...
    def candidates_handler(self):
        return 'Yael', 'Sahar', 'Yael', 'Roy'

    def run(self, areas: dict = None) -> pd.DataFrame:
        ...
        self.ISC_table = ISC.ISC_by_events(self.events_table, self.subject_A.hbo_data.get_hbo_data(),
                                           self.subject_B.hbo_data.get_hbo_data(),by_areas=areas)

        A_pre_choice, B_pre_choice, post_choice, control = self.candidates_handler()

...
# creating a new DF with index and timestamps, which are coulmns 0, 1 and new columns as the lables. will start with initialized value of 'False' per each label.    
    ...
    for index, row in df.iterrows():
        ...
        if (row['KP_0_x'] < 0 ) or (row['K_15_y'] > 0) or (row['K_17_y']>0):
        ...
        if (row['KP_0_x'] > 0 ) or (row['K_16_y'] > 0) or (row['K_18_y'] > 0):
        ...
        if row['KP_02_y']< -T_Hold:
            if (row['KP_0_x'] < 0 ) or (row['K_16_x'] <0) or (row['K_18_x']<0):
                df_new['ROUND_R'] = True 
        if row['KP_05_y']< -T_Hold:
